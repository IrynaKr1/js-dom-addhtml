...
function createHeaderEl(news) {
  const headerEl = document.createElement('header');
  headerEl.classList.add('newsHeader');
  const img = createBgImg(news);
  const title = createTitleEl(news);
  const buttons = createDivBtnEl(news);
  headerEl.append(img, title, buttons);
  return headerEl;
}

function createArticleEl(news) {
  const articleEl = document.createElement('article');
  articleEl.classList.add('newsArticle');

  const header = createHeaderEl(news);
  const category = createCategoryEl(news);
  const text = createTextEl(news);
...
const newsArticles = news.map((n) => createArticleEl(n));
document.body.append(...newsArticles);
